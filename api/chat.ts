...
const APP_TITLE = process.env.APP_TITLE || 'AI Fitness App'
const APP_REFERER_URL = process.env.APP_REFFER_URL || 'http://localhost:5173'
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    return res.status(400).json(`{ ${err} 'Invalid JSON in body.' }`)
  }

  const { userMessage, userProfileData, trainerMetaData } = body
